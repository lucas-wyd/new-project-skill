...
Establish the project location first. Reuse a directory explicitly chosen by the user for this project; otherwise ask where to store it before product-discovery questions, project inspection, or writes. An automatically assigned working directory is only a candidate, not the user's choice. Ask the location question on its own, allow a custom path, and wait for the submitted answer using [the interactive-question protocol](references/collaboration.md#interactive-questions).

Once the location is established, resolve the immediate request from conversation and local evidence. Distinguish the project directory from an enclosing Git repository. Inspect applicable instructions, relevant top-level docs, Git root/worktree, uncommitted changes, remotes, and existing plans. Inspect a missing directory's parent; if the path is a file or the intended target is consequentially ambiguous, resolve that before writing. Inspect secret-bearing files by name only.
...
For a completed handoff or user-requested stop, end with the project path, stage reached, deliverable links, verification, and the exact pending decision or next action. At an interactive checkpoint, show that context before asking, then follow [the interactive-question protocol](references/collaboration.md#interactive-questions) instead of sending a closing summary while the answer is pending. Never label a draft approved, an approved plan authorized for execution without evidence, or planned work implemented.
